...
type GameMapFixture = {
  zones: Array<{ id: string; kind: string; name: string; herdGroupId: string | null; ring: Array<{ lat: number; lng: number }> }>;
...
/**
 * Garante um mapa mínimo do jogo (perímetro + 1 pasto vinculado ao primeiro
 * lote ativo + mangueira) criado direto pela API — rápido e independente do
 * editor. Reutiliza o que já existir (padrão serial do repo).
 */
export async function createGameMapFixture(page: Page): Promise<GameMapFixture> {
  return page.evaluate(async (ring) => {
    ...
    if (!state.zones.some((zone) => zone.kind === 'PASTURE')) {
      ...
      await fetch('/api/game/map/zones', json({ kind: 'PASTURE', name: 'Pasto da Frente', herdGroupId: groups.find((group) => group.active)?.id ?? null, ring: inner }));
    }
