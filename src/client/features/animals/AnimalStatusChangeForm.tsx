...
export function AnimalStatusChangeForm({ animalId, currentStatus, initialStatus, onSaved, onCancel }: Props) {
  ...
  async function persist() {
    const { changedOn, notes, groupId, exitType, reason, buyerName, weightKg, amount, createRevenue, existingRevenueId } = form.values;
  ...
  return <form className="grid gap-3" noValidate onSubmit={(event) => { event.preventDefault(); if (form.validate()) void run(persist); }}>
    ...
    {statusRequiresMilkingGroup(status) && <GroupPicker label="Lote de ordenha" value={form.values.groupId} fieldError={form.error('groupId')} onChange={(value) => form.set('groupId', value)} />}
