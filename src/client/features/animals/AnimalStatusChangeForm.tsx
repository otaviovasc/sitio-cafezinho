...
export function AnimalStatusChangeForm({ animalId, currentStatus, initialStatus, onSaved, onCancel }: Props) {
  ...
  return <form className="grid gap-3" onSubmit={submit}>
    ...
    <div className="flex flex-wrap gap-2"><Button type="submit" disabled={busy || status === currentStatus || (statusRequiresMilkingGroup(status) && !groupId) || ((status === 'SOLD' || status === 'DEAD') && !notes.trim()) || (status === 'SOLD' && createRevenue && !amount)}>{busy ? 'Registrando…' : status === 'LACTATING' ? 'Registrar parto e iniciar lactação' : 'Registrar mudança'}</Button>{onCancel && <Button type="button" variant="secondary" onClick={onCancel}>Cancelar</Button>}</div>
  </form>;
}
