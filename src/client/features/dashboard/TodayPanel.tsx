import { Activity, AlertTriangle, BadgeDollarSign, Banknote, CalendarClock, CheckCircle2, Download, FileText, Milk, Scale, Search, ShoppingCart, Store, Truck, Upload, WalletCards, Wheat } from 'lucide-react';
import { Link } from 'react-router-dom';
import { formatDate, formatLiters, formatMoney } from '../../../domain/format';
import { CowHead } from '../../components/icons';
import { Badge, ErrorState, SectionCard, SkeletonList, StatCard } from '../../components/ui';
...
/**
 * Painel "Hoje": o conteúdo do antigo dashboard (fechamento do dia, follow-ups,
 * carências informadas, resumo diário e visão mensal) como componente
 * reutilizável. Montado na aba Hoje do Caderno do jogo — o registro por voz/
 * foto fica no MicFab global, por isso o CaptureCard não faz parte daqui.
 */
export function TodayPanel() {
  const { data, loading, error, reload } = useResource<Dashboard>('/api/dashboard');
  if (loading) return <SkeletonList rows={4} />;
  if (error || !data) return <ErrorState message={error || 'Painel indisponível.'} retry={reload} />;
  ...
  return <div className="grid gap-5">
    <p className="text-sm text-[var(--muted)]">{formatDate(data.date)} · o que precisa ser registrado ou resolvido</p>
