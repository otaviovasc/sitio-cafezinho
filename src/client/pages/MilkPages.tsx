...
export function MilkSessionsPage() {
  ...
  return <div className="page">
    <PageHeader icon={Milk} title="Produção" subtitle="Em cada dia, registre o total do rebanho ou importe um controle individual completo" action={<Link className="button button-primary" to="/producao/importar"><Plus size={18} aria-hidden />Importar controle</Link>} />
    <div className="grid grid-cols-1 gap-5"><SectionCard icon={ChartNoAxesCombined} title="Evolução da produção"><PeriodSelector value={period} onChange={setPeriod} /><p className="mb-3 mt-3 text-xs text-[var(--muted)]">A linha combina o total diário e, nos dias de controle completo, a soma das medições individuais. Dias sem registro permanecem ausentes.</p>{timelineLoading ? <LoadingState /> : timelineError ? <ErrorState message={timelineError} retry={reloadTimeline} /> : <TimeSeriesChart data={chartData} series={[{ key: 'totalLiters', label: 'Produção do rebanho', color: '#315c3b', area: true }]} label="Evolução da produção total do rebanho no período selecionado" />}</SectionCard><DailyMilkPanel onChange={reloadTimeline} /><section className="min-w-0"><div className="mb-3 flex flex-wrap items-center justify-between gap-2"><h2 className="text-xl font-bold">Controles individuais completos</h2><Link className="button button-secondary" to="/producao/importar">Importar do ChatGPT</Link></div>
