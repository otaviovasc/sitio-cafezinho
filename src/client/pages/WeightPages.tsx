...
import { Badge, Button, EmptyState, ErrorState, Field, FilterBar, InlineEmpty, Input, PageHeader, ScrollArea, SectionCard, Select, SkeletonList, StatCard, StatusBadge, Textarea } from '../components/ui';
...
function WeightReview({ rows, setRows, animals, busy, onConfirm }: { rows: WeightRow[]; setRows: (rows: WeightRow[]) => void; animals: Animal[]; busy: boolean; onConfirm: () => void }) {
  ...
  return <SectionCard title="3. Revisar antes de salvar" icon={ClipboardCheck}>
    ...
    {!visible.length ? <InlineEmpty className="mt-4">Nenhuma linha encontrada com estes filtros.</InlineEmpty> : <ScrollArea label="Revisão das pesagens" className="mt-4 max-h-[42rem]">{visible.map(({ row, index }) => <div className={`review-row ${row.status === 'NEEDS_REVIEW' ? 'review-row-warning' : ''}`} key={`${row.rawAnimalLabel}-${index}`}>
      <div className="flex items-start justify-between gap-3"><div><strong>{row.rawAnimalLabel}</strong><p className="text-xs text-[var(--muted)]">Original preservado{row.rawValueText ? ` · “${row.rawValueText}”` : ''}</p></div><StatusBadge descriptor={weightMeasurementStatusDescriptor[row.status]} /></div>
...
export function ImportWeightsPage() {
  ...
  return <div className="page"><PageHeader icon={Scale} title="Nova pesagem" subtitle="Pode ser parcial: registre somente os animais realmente pesados" />
    <div className="grid gap-5">
      ...
      <SectionCard title="2. Colar e validar"><Field label="JSON da transcrição"><Textarea className="min-h-64 font-mono text-sm" value={content} onChange={(event) => { setContent(event.target.value); setPreview(null); }} /></Field><div className="mt-3 flex flex-wrap gap-2"><Button variant="secondary" onClick={() => setContent(weightExample(date))}>Carregar exemplo</Button><Button disabled={busy || !content.trim()} onClick={() => void validateRows()}>{busy ? 'Validando…' : 'Validar pesagens'}</Button></div></SectionCard>
